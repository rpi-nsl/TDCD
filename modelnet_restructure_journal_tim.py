...
#from models.resnet2_modelnet import *
...
def parse_args():
    ...
    parser.add_argument('--hubs', type=int, nargs='?', default=12,
    ...
    parser.add_argument('--modelnet_type', type=str, nargs='?', default="10",
                            help='Use ModelNet10 or ModelNet40')
...
if __name__ == "__main__":
    ...
    modelnet_10_dir = "10class/classes/"
    if args.modelnet_type == "40":
        modelnet_10_dir = "view/classes/"
    print("Loading from directory", modelnet_10_dir)
    ...
    in this case, for modelnet10/mnist, we have 
    https://github.com/kuangliu/pytorch-modelnet/issues/19
    ...
    """
     ...
     then distribute the extra coordinates from first partition onwards
    ...
    coordinate_per_dc = int(X_train.shape[WHICH_COORDINATE_INDEX_TO_DIST]/N)# Get the shape from the last column
    extradatapointsinfirstdevice = X_train.shape[WHICH_COORDINATE_INDEX_TO_DIST] - coordinate_per_dc*N
    i = 0
    ...
    over_train_loader = torch.utils.data.DataLoader(CustomTensorDataset(tensors=(X_train, y_train), transform=None)
                                    , batch_size=int(args.batchsize/N), shuffle=False)
    ...
    # ./results/
    PATH = (f"Checkpoint_Modelnet_model_BS{local_batch_size}_N{N}_K{K}_Q{local_epoch}_lr{alpha}_momentum{momentum}_seed{args.seed}_sampling{args.withreplacement}_modelnet{args.modelnet_type}.pt")
    if os.path.exists(PATH):
        ...
        if not args.stepLR:
            ...
            filename =f"Modelnet_model_BS{local_batch_size}_N{N}_K{K}_Q{local_epoch}_lr[{alpha},0.005,0.001]_momentum{momentum}_seed{args.seed}_sampling{args.withreplacement}_eval{args.evaluateateveryiteration}_evalafter{args.evalafter}_modelnet{args.modelnet_type}.pkl" 
    ...
    for t in tqdm(range(START_EPOCH, global_epoch)):
        ...
        for iteration in range(local_epoch):      
            ...
            if args.evaluateateveryiteration or (t*local_epoch + iteration+1)%args.evalafter==0:
                ...
                get_train_or_test_loss_generic([averaged_networks[i]for i in range(len(averaged_networks))], 
                                       over_train_loader, over_test_loader, report, coordinate_partitions=coordinate_partitions) # i.e.divide each image at WIDTH column

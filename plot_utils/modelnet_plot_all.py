...
seed = 2021
cutoff = [380,330]
for modelnet_type in [40]:
    for seedidx, seed in enumerate([2021]):
        for iii, K_val in enumerate([10, 20]):    
            fig=plt.figure(figsize=(10,6), dpi= 100, facecolor='w', edgecolor='k')                
            for q, Q_val in enumerate([1, 10, 20, 30]):    
                one_f = open(os.path.join(FOLDER, f"Modelnet_model_BS320_N12_K{K_val}_Q{Q_val}_lr{lr}_momentum0.0_seed{seed}_samplingFalse_evalFalse_evalafter20.0_modelnet{modelnet_type}.pkl"),'rb')
                print(f"Modelnet_model_BS320_N12_K{K_val}_Q{Q_val}_lr{lr}_momentum0.0_seed{seed}_samplingFalse_evalFalse_evalafter20.0_modelnet{modelnet_type}.pkl")
                one = pickle.load(one_f)
                loss = np.array([acc.item() for acc in one['test_accuracy5']])[:int(cutoff[iii])]
            ...
            #plt.show()
            plt.savefig(f'modelnet{modelnet_type}_testacc_K{K_val}_lr{lr}_seed{seed}.pdf')
    ...
    for seedidx, seed in enumerate([2021]):
        for iii, K_val in enumerate([10, 20]):    
            ...
            for q, Q_val in enumerate([1, 10, 20, 30]):    
                one_f = open(os.path.join(FOLDER, f"Modelnet_model_BS320_N12_K{K_val}_Q{Q_val}_lr{lr}_momentum0.0_seed{seed}_samplingFalse_evalFalse_evalafter20.0_modelnet{modelnet_type}.pkl"),'rb')
                one = pickle.load(one_f)
                loss = np.squeeze(one["train_loss"])[:int(cutoff[iii])]
                skipsteps=3 # i.e. print loss after every 60 rounds.
                loss = loss[::skipsteps]    
                timeline = np.arange(len(loss))
                plt.plot(timeline*20*skipsteps, loss, label=f"Q:{Q_val}", linewidth=2, c=colors[q])
            ...
            #plt.show()
            plt.savefig(f'modelnet{modelnet_type}_trainloss_K{K_val}_Q{Q_val}_seed{seed}.pdf')
    ...
    for comm in [10, 100]:    
        for iii, K_val in enumerate([10, 20]):    
            ...
            for q, Q_val in enumerate([1, 10, 20, 30]):    
                one_f = open(os.path.join(FOLDER, f"Modelnet_model_BS320_N12_K{K_val}_Q{Q_val}_lr{lr}_momentum0.0_seed{seed}_samplingFalse_evalFalse_evalafter20.0_modelnet{modelnet_type}.pkl"),'rb')
                one = pickle.load(one_f)
                metric = np.array([acc.item() for acc in one['test_accuracy5']])
            ...
            plt.ylabel("Test Accuracy", fontsize=18)
            plt.savefig(f'modelnet{modelnet_type}_testacc_latency{comm}_K{K_val}_Q{Q_val}_seed{seed}.pdf')
